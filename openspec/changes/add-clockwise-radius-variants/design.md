...
当前单图多轨迹批处理把 preset 目录编号、manifest 键和值班输出都绑定在固定的 canonical preset 列表上。现有 orbit 逻辑只有一个 `clockwise` 分支, 它通过 `_generate_clockwise_offsets_cv(...)` 结合全局 `radius_x_factor` / `radius_y_factor` 生成环绕位移。

这次需求不是新增一个可自由配置的轨迹系统, 而是在现有 deterministic preset 工作流里补两个用户可直接选择的顺时针环绕档位。因此设计重点不是“做出更多参数”, 而是“在不破坏现有 preset 索引和语义的前提下, 把半径尺度差异收进 preset 元数据”。

## Goals / Non-Goals

**Goals:**
- 为单图多轨迹批处理增加 `clockwise_0.65` 和 `clockwise_1.5` 两个具名 orbit preset。
- 保持现有 `clockwise` 的索引、语义、运动距离和用户使用方式不变。
- 让新增 preset 自动进入 canonical 列表、subset 选择、dry-run 展示、manifest 命名和测试覆盖。
- 用数据驱动的方式表达 orbit 半径倍率, 避免把更多特殊分支堆进轨迹生成逻辑。
...
### 1. 在 preset 元数据中显式保存 orbit 半径倍率
- 决定: 为 orbit preset 增加一个与名称解耦的半径倍率元数据, 由 `clockwise = 1.0`, `clockwise_0.65 = 0.65`, `clockwise_1.5 = 1.5` 表达差异。
...
### 2. 复用现有 `clockwise` 的运动距离, 只缩放 orbit 半径
- 决定: 两个新 preset 继续使用现有 `clockwise` 的 `movement_distance_range`, 只在 orbit 位移计算时按倍率同步缩放 `radius_x_factor` 和 `radius_y_factor`.
- 理由: 用户需求说的是“半径是现在 `clockwise` 的 0.65 / 1.5 倍”, 不是要求不同的总运动距离。复用现有 movement distance 才能保持“只有半径变了, 其他都没变”。
- 备选方案: 通过修改 `movement_distance_range` 来间接拉大或缩小轨迹.
  - 放弃原因: 这会同时影响位移尺度和其它推导量, 不再是纯粹的半径倍率语义。

### 3. 保持现有索引稳定, 新 preset 追加到尾部
- 决定: 继续保留 `left..clockwise` 的现有索引 `0..5`, 新增 `clockwise_0.65 = 6`, `clockwise_1.5 = 7`.
...
- [行为误改] → 如果实现时误动了原有 `clockwise` 的默认倍率, 用户历史结果会漂移. 缓解: 用比例测试和稳定索引测试同时锁住基准行为。
- [文档偏移] → 代码变成 8 个 preset 后, README 若仍写 6 个会误导用户. 缓解: 把 README 更新列入同一批任务与验证范围。

## Migration Plan

- 代码层没有持久化数据迁移需求。
- 新版本开始, 默认全量批处理会从 6 条 preset 扩展到 8 条。
