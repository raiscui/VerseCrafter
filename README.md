...
2. **Create and activate the Pixi environment:**
   ...
   - `install-grounded-sam2` now uses `--no-build-isolation`, so it reuses the Pixi-managed `torch` and `setuptools` already present in the environment instead of downloading another isolated build environment during bootstrap. It also limits compile parallelism by default, keeps at least 4 CPU cores free, and forwards the computed worker count to both `MAX_JOBS` and `CMAKE_BUILD_PARALLEL_LEVEL`. You can tune `SAM2_RESERVED_CPUS` in `.envrc`, or override `MAX_JOBS` manually if you need a fixed worker count.
   - `install-flash-attn` now limits compile parallelism by default, keeps at least 4 CPU cores free, and only builds the currently detected GPU architecture instead of compiling all default CUDA targets. You can tune `FLASH_ATTN_RESERVED_CPUS` in `.envrc`, override `MAX_JOBS` manually if you need a fixed build parallelism, or set `FLASH_ATTN_CUDA_ARCHS` yourself if you need a custom target.
   - `install-pytorch3d` keeps the source build path, but now follows the released `stable` Git ref from the official PyTorch3D install guide by default instead of always tracking `main`. The script still uses a shallow clone to reduce GitHub download time and proxy traffic, limits compile parallelism by default, keeps at least 4 CPU cores free, and forwards the computed worker count to both `MAX_JOBS` and `CMAKE_BUILD_PARALLEL_LEVEL`.
   - `install-pytorch3d` now also short-circuits when `pytorch3d` is already installed, so rerunning `bootstrap` does not automatically trigger another full C++ / CUDA rebuild. If you really need to rebuild it, set `PYTORCH3D_FORCE_REINSTALL=1`.
   - For newer CUDA stacks, `install-pytorch3d` now prefers the system compiler by default, attempts to infer `CUDA_HOME`, and exposes explicit overrides in `.envrc`: `PYTORCH3D_GIT_REF`, `PYTORCH3D_CUDA_HOME`, and `PYTORCH3D_TORCH_CUDA_ARCH_LIST`.
   - If you explicitly set `PYTORCH3D_CUDA_HOME`, it must point to the real CUDA toolkit root which actually contains `bin/nvcc`. A safe way to derive it is `export PYTORCH3D_CUDA_HOME="$(dirname "$(dirname "$(command -v nvcc)")")"`.
   - For Blackwell-class GPUs in the bundled Pixi environment, leaving `PYTORCH3D_TORCH_CUDA_ARCH_LIST` empty is usually enough. When the script sees a single `12.0` capability together with `torch 2.7+` and `CUDA 12.8+`, it auto-sets `TORCH_CUDA_ARCH_LIST=12.0` for the PyTorch3D build. If you are using a different newer torch runtime and want to force it manually, you can still export `PYTORCH3D_TORCH_CUDA_ARCH_LIST=12.0` before running `pixi run install-pytorch3d`.
