...
## [2026-03-24 07:43:12 UTC] [Session ID: codex-bootstrap-20260324-074312] 笔记: `pixi run bootstrap` 启动链路梳理

## 来源

### 来源1: `pixi.toml`
- 路径: `/root/autodl-tmp/home/rais/VerseCrafter/pixi.toml`
- 要点:
  - `bootstrap` 不是单独 shell 脚本, 而是一个 `depends-on` 聚合任务.
  - 依赖链固定为:
    - `install-moge`
    ...
    - `install-flash-attn`
    - `install-pytorch3d`
  - 每个子任务本质上都是 `python -m pip install ...`
  - 其中最可能受网络影响的是:
...
### 来源2: 历史归档上下文
- 路径: `/root/autodl-tmp/home/rais/VerseCrafter/archive/notes_2026-03-15_1705.md`
- 要点:
  - 仓库在改造 Pixi 工作流时, 已把 README 安装路径收敛为:
    - `pixi install`
    - `pixi run bootstrap`
    - `pixi shell`
  - 历史上只验证过 `pixi task list`, 还没有真正跑通完整 `bootstrap`.

## 综合发现

### 结构判断
- 当前任务更像“5 个特殊安装步骤的串行集成验证”.
- 所以后续日志分析要以“哪个子任务失败了”为主线, 而不是把 `bootstrap` 当成一个整体黑盒.

### 代理策略
- 用户已明确要求:
  - 先直连.
  - 只有遇到真实网络问题时才临时启用代理.
- 因此第一轮执行不应预设代理, 要先拿到真实失败证据.

## [2026-03-24 08:00:40 UTC] [Session ID: codex-bootstrap-20260324-074312] 笔记: `pixi run` 落到外部 Conda 的根因确认

## 来源

### 来源1: 直连运行时的动态探针
- 命令:
  - `pixi run python -c "import sys, site, pip; ..."`
...
### 来源2: `pixi install` 的失败输出
- 命令:
  - `pixi install`
- 关键输出:
  - `Failed to query Python interpreter at .../.pixi/envs/default/bin/python3.11`
  - `Permission denied (os error 13)`

### 来源3: `.pixi/envs/default/bin` 权限检查
- 命令:
  - `find .pixi/envs/default/bin -maxdepth 1 -type f | wc -l`
  - `find .pixi/envs/default/bin -maxdepth 1 -type f ! -perm /111 | wc -l`
  - `stat -c '%A %a %n' .pixi/envs/default/bin/python3.11 .pixi/envs/default/bin/git .pixi/envs/default/bin/pip`
- 观察:
  - 常规文件总数:
    - `318`
  - 缺执行位的常规文件数:
...
### 结论
- 已验证结论:
  - 当前真正阻塞 `pixi run bootstrap` 正确运行的原因, 是 `.pixi/envs/default/bin` 下整批可执行文件都丢失了执行位.
  - 因为项目环境里的 `python` / `pip` / `git` 都不可执行, `pixi run` 才退回系统 PATH, 最终跑到了外部 Conda Python 3.12.

### 最小修复实验
- 执行:
  - `find .pixi/envs/default/bin -maxdepth 1 -type f -exec chmod 755 {} +`
- 修复后验证:
  - `pixi run which python` -> `.../.pixi/envs/default/bin/python`
  - `pixi run which pip` -> `.../.pixi/envs/default/bin/pip`
  - `pixi run python -c "import sys; print(sys.executable); print(sys.version)"` -> Python `3.11.9`
  - `pixi install` -> `✔ The default environment has been installed.`

### 备选解释回滚
- 上一轮可能的备选解释是:
  - “也许只是 `python3.11` 这一个文件坏了”
- 现已被证据推翻:
  - `318 / 318` 个 bin 常规文件都缺执行位
  - 不是单文件故障

## [2026-03-24 08:03:10 UTC] [Session ID: codex-bootstrap-20260324-074312] 笔记: Git HTTPS helper 也受同类权限问题影响

## 来源

### 来源1: 第二轮 `pixi run bootstrap`
- 关键报错:
...
### 来源2: Git helper 路径与权限
- 命令:
  - `pixi run git --exec-path`
  - `stat -c '%A %a %n' .pixi/envs/default/libexec/git-core/git-remote-http .pixi/envs/default/libexec/git-core/git-http-fetch`
- 观察:
  - exec-path 是:
    - `.pixi/envs/default/libexec/git-core`
  - `git-remote-http` / `git-http-fetch` 初始权限都是:
    - `644`

### 来源3: 最小 HTTPS 验证
- 修复:
  - `find .pixi/envs/default/libexec -type f -exec chmod 755 {} +`
- 验证命令:
  - `pixi run git ls-remote https://github.com/microsoft/MoGe.git HEAD`
- 结果:
  - 成功返回 `07444410f1e33f402353b99d6ccd26bd31e469e8`

## 综合发现

### 结论
- `.pixi` 环境的权限问题不只影响 `bin/`, 也影响 `libexec/`.
- `git remote-https` 失败不是网络问题, 也不是 GitHub 拒绝访问, 而是本地 Git helper 不可执行.

## [2026-03-24 08:10:20 UTC] [Session ID: codex-bootstrap-20260324-074312] 笔记: `install-grounded-sam2` 需要显式关闭 build isolation

## 来源

### 来源1: 第二轮后半段动态进程证据
- `pixi run bootstrap` 进入 `install-grounded-sam2` 后, 长时间停在 spinner.
- 继续查子进程得到:
  - `python .../__pip-runner__.py install --ignore-installed --prefix /tmp/pip-build-env-... setuptools>=62.3.0,<75.9 torch>=2.3.1`
- 说明 pip 并没有直接复用当前 Pixi 环境, 而是在 build isolation 里重新装 build-system 依赖.

### 来源2: `third_party/Grounded-SAM-2/pyproject.toml`
- `build-system.requires` 明确包含:
  - `setuptools>=62.3.0,<75.9`
  - `torch>=2.3.1`

### 来源3: 当前 `pixi.toml` 与历史归档
- 原始配置里:
  - `install-grounded-sam2 = "python -m pip install -e ./third_party/Grounded-SAM-2"`
- 同时旁边注释却写着:
  - `仍保留原始 editable / no-build-isolation 语义`
- 归档任务也明确记过:
  - 这些 bootstrap 子任务里, 特殊安装方式要尽量保留原始语义.

## 综合发现

### 结论
- 对 `Grounded-SAM-2` 来说, 普通 `pip install -e` 会触发 build isolation.
- 由于它的 `build-system.requires` 包含 `torch>=2.3.1`, pip 会在临时环境里再装一遍 torch, 这不是当前 Pixi 工作流想要的路径.
- 因此把 `install-grounded-sam2` 改成:
  - `python -m pip install --no-build-isolation -e ./third_party/Grounded-SAM-2`
  是有静态和动态证据支撑的改正, 不是猜测性优化.
