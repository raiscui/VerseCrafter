...
## [2026-03-22 11:06:00 UTC] 排查 `my4` 双卡命令里“只有一张 GPU 在工作”

### 目标
- 判断用户给出的 `single_image_multi_trajectory.py` 双卡命令本身是否正常.
- 区分“脚本设计上某些阶段本来单卡”与“Step 6 双卡没有真正生效”这两类情况.
- 给出基于当前机器和 `demo_data/my4` 产物状态的证据结论.

### 阶段
- [x] 阶段1: 回读历史排障记录与当前多卡预检逻辑
- [x] 阶段2: 检查当前机器的 GPU / Torch 可见性
- [x] 阶段3: 用 dry-run 核对 `demo_data/my4` 这条命令当前会执行哪些阶段
- [x] 阶段4: 汇总结论并答复用户

### 验证
- `nvidia-smi`
  - 结果: 当前机器可见 2 张 A800
- `pixi run python - <<'PY' ... torch.cuda.device_count() ... PY`
  - 结果: `torch.cuda.device_count = 2`
- 对用户命令做 `--dry_run`
  - 结果:
    - `0..5` 的 generation 为 `reuse`
    - `6` 为 `generation: run`
    - `7..11` 为 `render: run` 且 `generation: run`
    - render 命令是单卡 `--device cuda`
    - generation 命令才是 `torchrun --nproc-per-node=2`

### 状态
**任务已完成**
- 已确认命令的双卡参数本身正常.
- 看到单卡占用并不自动等于“双卡失效”, 需要先看当前处于 Step 5 还是 Step 6.

## [2026-03-22 11:12:30 UTC] 继续排查: 为什么跑到 8 号镜头仍只见 1 张 GPU 工作

### 目标
- 用动态证据确认当前正在跑的到底是:
  - Step 5 render
  - 还是 Step 6 双卡 generation
- 判断“8 号镜头只见 1 张 GPU 工作”是脚本设计使然, 还是 Step 6 双卡没有真正起起来.

### 阶段
- [ ] 阶段1: 抓当前进程与 GPU 活动快照
- [ ] 阶段2: 检查 `demo_data/my4/8` 的产物状态与时间戳
- [ ] 阶段3: 汇总现象、假设、验证结论并答复用户

### 当前动作
- 正在读取当前系统里的 `torchrun` / `versecrafter_inference.py` / `rendering_4D_control_maps.py` 进程.
- 正在抓 `nvidia-smi pmon` 与 `demo_data/my4/8` 的目录状态.

### 状态
**目前在阶段1-2**
- 需要先拿到动态证据, 再判断是不是双卡异常.

## [2026-03-22 12:16:00 UTC] 新证据: 当前 8 号镜头实际拉起的是单卡 Step 6 进程

### 新观察
- 当前正在运行的子进程不是 `torchrun`.
- 实际看到的是:
  - `inference/versecrafter_inference.py --save_path demo_data/my4/8/generated_videos ...`
- 并且这个子进程参数里显示的是:
...
### 含义
- 这和用户口头给出的命令不一致.
- 所以“8 号镜头为什么只有 1 张 GPU 工作”的最新主假设已经变化:
  - 不是双卡 Step 6 没有吃满
  - 而是当前实际跑起来的 `8` 号 Step 6 子进程本身就是单卡参数

### 当前动作
- 正在继续读取父进程完整命令行和 manifest settings.
- 目标是确认:
  - 是这次启动命令本身就没有把双卡参数传进去
  - 还是用户当前观察到的是另一轮旧进程
