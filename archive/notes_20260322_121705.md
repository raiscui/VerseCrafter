...
## [2026-03-22 11:06:00 UTC] `my4` 双卡命令分析: 为什么看起来只有一张 GPU 在工作
...
### 静态证据
- `single_image_multi_trajectory.py` 中:
  - Step 1 `build_moge_command()` 走的是单进程 `python ... --device cuda`
  - Step 2 `build_segmentation_command()` 走的是单进程 `python ... --device cuda`
  - Step 3 `build_gaussian_command()` 走的是单进程 `python ... --device cuda`
  - Step 5 `build_render_command()` 走的是单进程 `python ... --device cuda`
  - 只有 Step 6 `build_generation_command()` 才会在 `nproc_per_node > 1` 时切到:
...
- 对用户命令做 `--dry_run` 后, `demo_data/my4` 的当前状态是:
  - `0..5` 轨迹: `generation: reuse`
  - `6`: 直接 `generation: run`
  - `7..11`: 先 `render: run`, 再 `generation: run`
- 这意味着:
  - 如果观察时正落在 `7..11` 的 render 阶段
  - 那只有一张 GPU 在忙是符合当前脚本设计的
  - 因为 Step 5 不是双卡
...
- 结论2:
  - “有一张显卡不工作” 并不一定是 bug.
  - 如果观察点落在 Step 1/2/3/5, 本来就只有单卡负载.
- 结论3:
  - 只有在真正进入 Step 6 的 `torchrun --nproc-per-node=2` 后, 如果第二张卡仍长期 0 利用率, 才值得继续怀疑双卡没有生效.
